...
- **守护进程**：监控代理软件退出，自动清理系统代理，防止"关代理就断网"问题
...
### proxy-guard.ps1（守护进程）

解决经典问题：**关掉代理软件 → 系统代理还开着 → 端口已死 → 断网**。

守护脚本监控 FlClash/Clash/Mihomo 进程，检测到所有代理软件退出后自动清理系统代理。

| 命令 | 说明 |
|------|------|
| `watch` | 启动守护，监控代理进程，退出时自动清理（默认） |
| `now` | 立即强制清理系统代理（应急救急） |
| `status` | 查看当前运行的代理进程和代理状态 |

**推荐用法：** 开机或打开代理软件后运行，保持后台即可：
```batch
